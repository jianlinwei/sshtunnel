...
//			lpf2 = connection.createLocalPortForwarder(1053, "8.8.4.4", 53);
	...
	private void finishConnection() {
		...
		try {
			if (enablePortForward()) {
				...
				if (isAutoSetProxy) {
					runRootCommand("/data/data/org.puff/proxy.sh start "
							+ localPort);

					if (isARMv6()) {
						runRootCommand("/data/data/org.puff/iptables_g1 -t nat -A OUTPUT -p tcp "
								+ "--dport 80 -j REDIRECT --to-ports 8123");
						runRootCommand("/data/data/org.puff/iptables_g1 -t nat -A OUTPUT -p tcp "
								+ "--dport 443 -j REDIRECT --to-ports 8124");
						runRootCommand("/data/data/org.puff/iptables_g1 -t nat -A OUTPUT -p udp "
								+ "--dport 53 -j REDIRECT --to-ports 8153");
					} else {
						runRootCommand("/data/data/org.puff/iptables_n1 -t nat -A OUTPUT -p tcp "
								+ "--dport 80 -j REDIRECT --to-ports 8123");
						runRootCommand("/data/data/org.puff/iptables_n1 -t nat -A OUTPUT -p tcp "
								+ "--dport 443 -j REDIRECT --to-ports 8124");
						runRootCommand("/data/data/org.puff/iptables_g1 -t nat -A OUTPUT -p udp "
	...
	public void onDestroy() {
		...
		if (isAutoSetProxy) {
			if (isARMv6()) {
				runRootCommand("/data/data/org.puff/iptables_g1 -t nat -F OUTPUT");
			} else {
				runRootCommand("/data/data/org.puff/iptables_n1 -t nat -F OUTPUT");
			}
